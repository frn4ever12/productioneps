...
const ListQuize = () => {
    ...
    const setToLocalStorage = (id, photo, heading, sub_heading, price, time_duration, created_by, tags) => {
    ...
    return (
        <div className="container mx-auto">
            <div className="flex justify-between items-center mb-4 mt-6">
                <div className="text-gray-700 sans-serif-text text-3xl ml-1">Quiz List</div>
                <div>
                    <Link to="/addquiz">
            ...
            <div className="overflow-auto shadow mx-26 mt-8">
                <table className="w-full mb-0">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {quizList.map(quiz => (
                            <tr key={quiz.id} className="bg-white border-r text-left border-b text-sm text-gray-600">
                                <td className="p-3 text-sm text-gray-700 whitespace-nowrap">{quiz.id}</td>
                                <td className="p-3 text-sm text-gray-700 whitespace-nowrap">
                                    <img src={quiz.photo} alt={`Quiz ${quiz.id}`} className="h-10 w-12 rounded-lg" />
                                </td>
                                <td className="p-3 text-sm text-gray-700 whitespace-nowrap">{quiz.heading}</td>
                                <td className="p-3 text-sm text-gray-700 whitespace-nowrap">{quiz.sub_heading}</td>
                                <td className="p-3 text-sm text-gray-700 whitespace-nowrap">{quiz.price}</td>
                                <td className="p-3 text-sm text-gray-700 whitespace-nowrap">{quiz.time_duration}</td>
                                <td className="p-3 text-sm text-gray-700 whitespace-nowrap">{quiz.created_by}</td>
                                <td className="p-3 text-sm text-gray-700 whitespace-nowrap">
                                    {quiz.tags.map(tag => (
                                        <span key={tag.id} className="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2">{tag.tage_name}</span>
                                    ))}
                                </td>
                                <td>
                                    <Link to={"/updatequize"}>
                                    ...
                                    <button onClick={() => handleDeleteQuiz(quiz.id)} className="bg-red-800 text-white px-3 py-2 rounded-lg hover:bg-red-900 hover:text-red">
